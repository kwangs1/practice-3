...
<%@include file="../common/nav.jsp"%>
			<table class="table">
				...
				<ul class="pagination">
				    ...
				    <li>
				        <a href='<c:url value="/board/lists?page=${pageMaker.startPage-1 }"/>'><i class="fa-solid fa-chevron-left"></i></a>
				    </li>
				    </c:if>
				    <c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }" var="pageNum">
				    <li>
				        <a href='<c:url value="/board/lists${pageMaker.makeQueryPage(pageNum) }"/>'><i class="fa">${pageNum }</i></a>
				    </li>
				    </c:forEach>
				    <c:if test="${pageMaker.next && pageMaker.endPage >0 }">
				    <li>
				        <a href='<c:url value="/board/lists${pageMaker.makeQueryPage(pageMaker.endPage+1) }"/>'><i class="fa-solid fa-chevron-right"></i></a>
				    </li>
				    </c:if>
				</ul>
	</div>
